...
fn eval<Host>(
    ...
    let span = context
        ...
        .clone()
        .src
        .and_then(|src| src.merge(Some(list.src.clone())))
...
fn to_list<Host>(
    ...
    let list_span = context
        ...
        .src
        .clone()
        .and_then(|src| src.merge(Some(span.clone())))
        .unwrap_or(Source::Null);
...
fn clone<Host>(
    ...
    expression.src = context
        ...
        .src
        .clone()
        .and_then(|src| src.merge(expression.src));
...
fn list<Host>(
    ...
    for _ in 0..len.inner {
        ...
        span = span.merge(item.src.clone()).unwrap_or(Source::Null);
        items.insert(0, item);
    }
...
fn map<Host>(
    ...
    let data = v::List::new(
        ...
        context
            ...
            .clone()
            .and_then(|src| {
                src.merge(Some(list.src))
                    .unwrap_or(Source::Null)
                    .merge(Some(function.src))
            })
            .unwrap_or(Source::Null),
    );
...
fn wrap<Host>(
    ...
    let span = context
        ...
        .src
        .clone()
        .and_then(|src| src.merge(arg.src.clone()))
        .unwrap_or(Source::Null);
    let list = v::List::new(
        value::ListInner::from_values(
            vec![arg],
